...
if(1) # Andrea
{
  ...
  out.dir <- '~/Documents/P1Brazil/hfr-fit-210719h2-resources8-ex0-trvaluecs-hd1'
...
args$file.SIVEP <- file.path(pkg.dir, 'inst/data', 'SIVEP_hospital_20-09-2021-all.RDS')
...
tmp <- function(x) split(x, cut(seq_along(x), 3, labels = FALSE))
...
if(length(args$selected.locs) >= 6){
  ...
  args$selected.locs.part1 <- tmp(args$selected.locs)[[1]]
  args$selected.locs.part2 <- tmp(args$selected.locs)[[2]]
  args$selected.locs.part3 <- tmp(args$selected.locs)[[3]]
}
...
if(args$make.preprocessing.plots)
{
	...
	if(args$partition.locs){
	  ...
	  ggsave(file=paste0(args$out.base,'_pop_prop_vaxcomp_part1.pdf'),p,w=10,h=12)
	  ggsave(file=paste0(args$out.base,'_pop_prop_vaxcomp_part1.png'),p,w=10,h=12)
	  ...
	  p <- ggplot(tmp1, aes(x = week.start)) +
	    ...
	    theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1),
	          ...
	          strip.background = element_blank(),
	          panel.spacing.y = unit(1, "lines"))
	  ggsave(file=paste0(args$out.base,'_pop_prop_vaxcomp_part2.pdf'),p,w=10,h=12)
	  ggsave(file=paste0(args$out.base,'_pop_prop_vaxcomp_part2.png'),p,w=10,h=12)
	  ...
	  p <- ggplot(tmp1, aes(x = week.start)) +
	    ...
	    theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1),
	          ...
	          strip.background = element_blank(),
	          panel.spacing.y = unit(1, "lines"))
	  ggsave(file=paste0(args$out.base,'_pop_prop_vaxcomp_part3.pdf'),p,w=10,h=12)
	  ggsave(file=paste0(args$out.base,'_pop_prop_vaxcomp_part3.png'),p,w=10,h=12)	
	}else{
	  tmp1 <- copy(tmp)
	  p <- ggplot(tmp1, aes(x = week.start)) +
	    geom_area(aes(y = prop,  alpha = Dose, fill = vaccine), position = 'stack') + # switch fill and alpha position depending on what u want
	    ...
	    theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1),
	          ...
	          strip.background = element_blank(),
	          panel.spacing.y = unit(1, "lines"))
	  ggsave(file=paste0(args$out.base,'_pop_prop_vaxcomp_part.pdf'),p,w=10,h=12)
	  ggsave(file=paste0(args$out.base,'_pop_prop_vaxcomp_part.png'),p,w=10,h=12)
	}
}
...
if(args$remove.unclassified.cases)
{
...
if(args$make.sexpr)
{
  ...
  tmp[, `:=` (`1-3`=format(round(`1-3`*100/Total,2), nsmall=2),
             ...
             Unknown=format(round(Unknown*100/Total,2), nsmall=2))  ]
  tmp[, `:=`(`1-3`=paste0(`1-3`, '%'),`4`= paste0(`4`, '%'), `5`=paste0(`5`, '%'), Unknown=paste0(`Unknown`, '%'))  ]
  #tmp <-merge(tmp2,tmp, by = 'loc_label')
  setcolorder(tmp, c('loc_label', 'Total', '1-3','4','5', 'Unknown'))
...
if(args$remove.unclassified.cases)
{
  ...
  set(dht, dht[,which(is.na(CLASSI_FIN))],'adm.cat','unknown')
}else{
  cat('\nsetting missing CLASSI_FIN to COVID19 susp/conf ...')
  set(dht, dht[, which(!is.na(CLASSI_FIN) & CLASSI_FIN%in%c(4,5))], 'adm.cat', 'COVID19 susp/conf')
  set(dht, dht[, which(!is.na(CLASSI_FIN) & CLASSI_FIN%in%c(1,2,3))], 'adm.cat', 'other')
}
...
{	
	...
	set(tmp, NULL, 'Hospital_type', tmp[, factor(Hospital_type, 
		...
		labels=c('difference to excess deaths', 
				 ...
				 'expected deaths among COVID-19 hospitalised patients with as of yet unknown outcome'))])
	...
	p <- ggplot(tmp) +    	
			...
			theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1), 
					strip.background = element_blank(),
					legend.position='bottom') +		
			labs(x='', y='observed and expected COVID-19 attributable deaths', fill='')
	ggsave(file=paste0(args$out.base,'_deathsn_over_time_byexpobs_2.pdf'),p,w=10,h=12, limitsize = FALSE)
	ggsave(file=paste0(args$out.base,'_deathsn_over_time_byexpobs_2.png'),p,w=10,h=12, limitsize = FALSE)
}
