...
{
  out.dir2 <- '~/Documents/P1Brazil/hfr-analyses/'
...
out.base2 <- file.path(out.dir2, file.prefix2)
############################################################
...
# read workspace
tmp <- list.files(out.dir2, pattern = 'allloc_workspace.rda', full.names = TRUE)[1]
load(tmp)
...
###############################################################################
# Chose whether or not want to consider P1 effect in counterfactural scenarios
###############################################################################
...
{
  # look for rds files
  files <- list.files(out.dir2, pattern = 'allloc(.*?)table.rds', full.names = TRUE)
  # tmp <- readRDS(files[1])
}else{
  # look for rds files
  files <- list.files(out.dir2, pattern = 'wildtype.rds', full.names = TRUE)
}
...
tmp2 <- readRDS(paste0(out.base2,'_minhfr_methods_table.rds'))
...
tmp2 <- dha[ loc.res.cat == 'resident' & grepl(args$keep.vac.type, vac_type) &
               DateAdmission >= w1.start & DateAdmission <= w2.end &
               adm.cat %in% c('COVID19 susp/conf', 'unknown') & hosp.cat=='in hospital',  
...
tmp2 <- readRDS(paste0(out.base2,'_counterfactual_hfr_minimum_across_locations.rds') )
...
if(P1.eff)
{
  saveRDS(tmp, paste0(out.base2, '_main_table1.rds'))
}else{
  saveRDS(tmp, paste0(out.base2, '_main_table1_noP1.rds'))
}


tmp1 <- readRDS(paste0(out.base2, '_main_table1.rds'))
# tmp1[1:2, paste0(overall.x, collapse=' ')]
# tmp1[1:2, paste0(gsub('%','\\\\%',overall.y), collapse=' ')]

tmp1 <- print(xtable::xtable(tmp), include.rownames=FALSE,  include.colnames = FALSE)
...
if(0)
{
  # Load this is sexpr was modified "manually"
  sexpr <- list.files(out.dir2, pattern = "substitute_expressions.rds", full.names = TRUE)
  sexpr <- readRDS(sexpr)
}
...
tmp1 <- unique(stringr::str_match(tmp$rn, pattern = '_(.*?)$')[,2])
tmp1 <- paste(as.character(lubridate::month(tmp1, label = TRUE, abbr=F)), year(tmp1))
...
rownames(tmp) <- stringr::str_match(rownames(tmp), pattern = 'r.(.*?)_')[,2]
...
rownames(tmp) <- tools::toTitleCase(rownames(tmp))
...
saveRDS(tmp, paste0(out.base2, '_resource_table.rds'))
...
tmp1 <- dcast(tmp1, loc_label~age.label,value.var = 'observed_deaths')
tmp1[,loc_label := change_city_label(loc_label)]
...
if(!merge12){
  saveRDS(tmp1, paste0(out.base2, '_hosps_deaths_supp.rds'))
}

#2
tmp <- subset(dht, Outcome2 %in% c('Death_Covid','Death_Covid_Estimated') & 
                loc.res.cat == 'resident' & grepl(args$keep.vac.type, vac_type) &
                DateAdmission >= w1.start & DateAdmission <= w2.end) 
if(merge12){tmp <- tmp[, age.label := change_age_label(age.label)]}
...
if(!merge12){
saveRDS(tmp2, paste0(out.base2, '_hosps_deaths_censadj_supp.rds'))
}

# 1,2
tmp <- merge(tmp1, tmp2, by = 'loc_label')
tmp <- tmp[, lapply(.SD, as.integer), by=loc_label]
saveRDS(tmp, paste0(out.base2, '_hosps_deaths_and_censadj_supp.rds') )

# 
tmp <- readRDS(file=paste0(out.base2,'_high_hfr_table.rds'))
...
if('manaus' %in% args$selected.locs)
{
  tmp <- paste0(out.base2,'_manaus_fit.rds')
...
tmp <- dha[ hosp.cat=='in hospital' & adm.cat %in% c('COVID19 susp/conf', 'unknown') &
...
dha[ hosp.cat=='in hospital' & adm.cat %in% c('COVID19 susp/conf', 'unknown') &
...
saveRDS(tmp, paste0(out.base2, '_extra_XX.rds') )
# tmp <- readRDS(paste0(out.base2, '_extra_XX.rds'))
...
saveRDS(blocks_numbers, paste0(out.base2, '_flowchart_numbers.rds'))
...
tmp1 <- as.data.table(readRDS(paste0(out.base2, '_main_table1.rds')))
...
saveRDS(counterfactual_all, paste0(out.base2, '_counterfactual_all.rds'))
