##################################################################################################
# Define the directories containing the results for Controlling Sampling and GISAID model running 
# -> Can be changed later when final outputs from the HPC have been run and are ready
##################################################################################################
results_1_dir <- '~/Documenvts/P1Brazil/hfr-analyses-random'
results_2_dir <- '~/Documents/P1Brazil/hfr-analyses'
results_string = c('Random or Controlled Sampling','GISAID')
...
{
  stop('Usage: Rscript sensitivity_analysis_ControlledSequenceDataCollection.R -dirs foo bar
       Preferable that exactly one of foo and bar contain the string "random" ')
} 
if(length(args_line) > 0) 
{
  stopifnot(args_line[[1]]=='-dirs')	
  tmp <- c(args_line[[2]],args_line[[3]])
  if(sum(grepl('random', tmp)) != 1)
  {
    warning("No provided directories contain 'random' in their name\n
            Run without arguments to see instructions")
    results_1_dir <- tmp[1]; results_2_dir <- tmp[2]
  }else{
    results_1_dir <- grep('random', tmp, value = TRUE)
    results_2_dir <- tmp[!grepl('random', tmp)]
  }
  
} 
out.dir2 <- results_1_dir
...
tmp <- data.table(loc_label=c('belo horizonte','manaus', 'sao paulo'))
...
#pos1 <- load_samples(do1, out.dir21, stan_data1)
...
pads5D$scenario[pads5D$scenario == "Random or Controlled Sampling"] <- "Random/Controlled\nSampling"
...
ggsave(file=paste0(out.base2, '_gamma_frequency.pdf'),p5D,w=10,h=6)
ggsave(file=paste0(out.base2, '_gamma_frequency.png'),p5D,w=10,h=6)
...
pads5B$scenario[pads5B$scenario == "Random or Controlled Sampling"] <- "Random/Controlled\nSampling"
...
pads_5C_processing <- function(pos){
  ...
  pads5AC <- merge(pads, tmp, by=c('loc_label','age.label'))
  
  
  pads5C <- subset(pads5AC, variable=='HFR_ratio' & CU-CL<2)
...
pads5C$scenario[pads5C$scenario == "Random or Controlled Sampling"] <- "Random/Controlled\nSampling"
...
p5C <- ggplot(pads5C, aes(x=age.label)) +
  ...
  geom_point(aes(y=M, colour=scenario), position=position_dodge(0.5)) +
  scale_colour_manual(values = c("#FFC759", "#9984D4")) +
  scale_y_continuous(expand=c(0,0), breaks=c(0, 1, 2), lim=c(0,2.2)) +
  facet_wrap(. ~ loc_label, ncol=4) +
  theme_bw() +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1),
        strip.background = element_blank(),
...
dhr <- dhr[dhr$loc_label %in% c('belo horizonte','manaus', 'sao paulo'), ]
...
p5A1$genomic <- "Random/Controlled\nSampling"
p5A2$genomic <- "GISAID"
...
ggsave(file=paste0(out.base2, '_effect_sizes.pdf'),p5,w=9,h=13)
ggsave(file=paste0(out.base2, '_effect_sizes.png'),p5,w=9,h=13)
