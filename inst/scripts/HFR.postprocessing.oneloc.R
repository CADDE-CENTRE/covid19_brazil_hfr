...
if(1) # Andrea
{
  ...
  out.dir <- '~/Documents/P1Brazil/hfr-fit-210719g2-sens-resources6-ex0-trvaluecs-hd1/'
...
for (age in unique(da$age.label)){
  ...
  # If just want to compare fits, look at this:
  # tmp3 <- data.table(age.label=age, week.idx=tmp3$week.idx,
  #                    hfr_loess30=predict(loess(hfr_empirical~week.idx, tmp3_noNA, span = 0.3), tmp3$week.idx),
  #                    hfr_loess40=predict(loess(hfr_empirical~week.idx, tmp3_noNA, span = 0.4), tmp3$week.idx),
  #                    hfr_loess50=predict(loess(hfr_empirical~week.idx, tmp3_noNA, span = 0.5), tmp3$week.idx),
  #                    hfr_loess75=predict(loess(hfr_empirical~week.idx, tmp3_noNA, span = 0.75),tmp3$week.idx))
