...
# PlotText requirements for submission
...
p <- ggplot(pads, aes(x=deaths_of_hospsi)) +	
    ...
    reqs
ggsave(file=paste0(args$out.base,'_allloc_deaths_ndeaths_fit_scatter.pdf'),p,w=15,h=18, dpi=350, units='cm')
ggsave(file=paste0(args$out.base,'_allloc_deaths_ndeaths_fit_scatter.png'),p,w=15,h=18, dpi=350, units='cm')
cat('DEATHS_NDEATHS_FIT_SCATTER\n')
...
ggsave(file=paste0(args$out.base,'_allloc_hospn_fit_scatter.pdf'),p,w=15,h=18, dpi=350, units='cm')
ggsave(file=paste0(args$out.base,'_allloc_hospn_fit_scatter.png'),p,w=15,h=18, dpi=350, units='cm')
cat('HOSPS_SCATTER\n')
...
if(0)
{
  tmp <- merge(tmp, unique(dh[, .(age.label,age.idx)]))
  dpop[, p:=pop/tpop]
  tmp <- merge(tmp, dpop[, .(loc_label, age.label, qpop)], by=c('loc_label', 'age.label'))
  tmp[, list( M = sum(M*qpop)), by='loc_label']
}
...
p <- ggplot(pads, aes(x=week.start)) +
		...
		geom_point(data=tmp, aes(y=tdeaths_of_hospsi), pch=18, size=0.5) +
		...
		facet_wrap(~change_city_label(loc_label), scales='free_y', ncol=4) +
		theme_bw() +
		theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1), 
				strip.background = element_blank(),
				legend.position='bottom',
				legend.key.size = unit(10, 'pt'),
				text=element_text(size=7, family = 'sans'), axis.text=element_text(size=6.5, family = 'sans')
				) +
    ...
    # + reqs 
ggsave(file=paste0(args$out.base,'_allloc_deaths_by_variant.pdf'),p, w=18,h=20, dpi=350, units='cm')
ggsave(file=paste0(args$out.base,'_allloc_deaths_by_variant.png'),p, w=18,h=20, dpi=350, units='cm')
cat('DEATHS_BY_VARIANT\n')
...
p <- ggplot(pads, aes(x=week.start)) +
		geom_vline(data=ddates, aes(xintercept=w2.start), colour='grey50') +
		geom_ribbon(aes(ymin=sM-M, ymax=sM, fill=age.label, alpha=variant)) +
		geom_point(data=tmp, aes(y=thosps), pch=18, size=0.5) +
...
ggsave(file=paste0(args$out.base,'_allloc_hosps_by_variant.pdf'),p, w=18,h=20, dpi=350, units='cm')
ggsave(file=paste0(args$out.base,'_allloc_hosps_by_variant.png'),p, w=18,h=20, dpi=350, units='cm')
cat('HOSPS_BY_VARIANT\n')
...
p <- ggplot(pads, aes(x=week.start)) +
    ...
    reqs
ggsave(file=paste0(args$out.base,'_allloc_seq_propP1_over_time.pdf'),p,w=15,h=12, dpi = 350, units='cm')
ggsave(file=paste0(args$out.base,'_allloc_seq_propP1_over_time.png'),p,w=15,h=12, dpi = 350, units='cm')
cat('SEQ_PROPP1\n')
...
diag <- list.files(args$out.dir , pattern='_diagnostics.csv')
...
{	
  ...
  cat('\nreading ',  gsub('^.*?hd1_(.*?)_diagnostics.csv$', '\\1', file), 'diagnostics: ')
  
  tmp <- as.data.table(read.csv(tmp), row.names = FALSE)
  tmp1 <- tmp[variable == 'min_ess_bulk', X1.]
  cat('min ESS: ', as.integer(tmp1), ', ')
  min_bulk_ESSs <- c(min_bulk_ESSs, tmp1)
  ...
  cat('max Rhat: ', round(tmp1, 3), '\n')
  max_Rhats <- c(max_Rhats, tmp1)
...
cat('\nThe average location-specific minimum bulk ESS was ', mean(min_bulk_ESSs), '\n')
cat('\nThe maximum Rhat was ', sexpr$max_Rhat, ' and was obtained for ', sexpr$max_Rhat_loc)
cat('\nThe average location-specific maximum Rhat was', mean(min_bulk_ESSs), '\n')
